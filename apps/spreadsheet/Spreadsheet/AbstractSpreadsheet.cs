...
public abstract class AbstractSpreadsheet
{
    ...
    /// 
    /// Adds the cell referred to by name to the ISet visited.
    /// 
    /// Loops through all the cells which are direct dependents of the cell
    /// referred to by name. If the cell referred to by name is involved in a circular dependency,
    /// throws a CircularException. Else it will visit all the direct dependents of the cell
    /// referred to by name and mark them as visited.
    /// 
    /// By calling Visit in the for loop it adds the visited nodes to changed in reverse order of when
    /// they were visited. This is important because this gives the correct order of which cell has to be
    /// modified first. 
    /// </summary>
